...
Mintlify 默认使用 3000 端口。你可以使用 `--port` 指定端口，例如：
...
每个 CLI 版本对应特定的 Mintlify 版本。如果本地预览与线上不一致，请更新 CLI：
...
<AccordionGroup>
  ...
  <Accordion title="问题：遇到未知错误">
    ...
    解决方案：前往设备根目录删除 `~/.mintlify` 文件夹，然后再次运行 `mint dev`。
  </Accordion>
</AccordionGroup>

想了解最新 CLI 版本的变更？请查看 [CLI changelog](https://www.npmjs.com/package/mintlify?activeTab=versions)。
